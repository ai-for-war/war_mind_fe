...
- `Markets`: "Stock Catalog" (path: `/stocks`) with a market-appropriate Lucide icon and "Backtest" (path: `/backtests`) with an analysis-appropriate Lucide icon
...
- **AND** the navigation items "Multi-Agent", "Stock Catalog", "Backtest", "Skill", "Interview Lab", "Meeting Recorder", "Voice Cloning", "Text to Image", and "Text to Speech" are visible with appropriate icons
...
The navigation menu SHALL highlight the item whose `url` matches the current route path. The active item SHALL be visually distinguished from inactive items using shadcn `SidebarMenuButton`'s `isActive` prop. This active-state behavior SHALL apply to grouped navigation destinations including `/multi-agent`, `/stocks`, `/backtests`, `/skill-plugins`, `/voice-cloning`, `/interview-lab`, `/meeting-recorder`, `/text-to-image`, and `/tts`.
...
#### Scenario: Backtest route is active
- **WHEN** the user is on the `/backtests` route
- **THEN** the "Backtest" navigation item is visually highlighted as active and nonmatching items are not
