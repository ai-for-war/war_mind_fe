...
} from "@/features/interview-lab/utils/interview-session.utils";
    ...
    <Collapsible open={isExpanded} onOpenChange={setIsExpanded} className="mt-2">
      <CollapsibleTrigger asChild>
        <Button
          type="button"
          variant="ghost"
          className="h-auto w-full justify-start px-0 py-0 text-left font-normal whitespace-normal hover:cursor-pointer hover:bg-transparent"
        >
          <p
            className={cn(
...
const InterviewPanelCard = ({
  children,
  desktop = false,
  title,
}: InterviewPanelCardProps) => {
  if (desktop) {
    return (
  ...
  return (
    <Card className="flex h-[44rem] max-h-[44rem] flex-col overflow-hidden border-border/60 bg-card/70">
      ...
      <CardContent className="flex min-h-0 flex-1 flex-col overflow-hidden">
        <ScrollArea className="h-full min-h-0 flex-1 pr-2">{children}</ScrollArea>
      </CardContent>
    </Card>
  );
};
...
export const InterviewLabPage = () => {
  ...
  const sessionControlPlaneContent = (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <div className="rounded-xl border border-border/60 bg-muted/20 p-4">
          ...
          <p className="mt-2 text-sm text-foreground">
            {acceptedConfig
              ? `${acceptedConfig.language} Â· ${acceptedConfig.encoding} Â· ${acceptedConfig.sampleRate}Hz Â· ${acceptedConfig.channels}ch`
              : `${selectedLanguage} Â· Awaiting stt:started`}
          </p>
        </div>
      </div>
  ...
  const transcriptMonitorContent = (
    <div className="space-y-3">
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between gap-3">
              ...
              <Badge variant={getStatusBadgeVariant(answer?.status ?? "idle")}>
                {answer?.status ?? "idle"}
              </Badge>
            </div>
  ...
  return (
    <section className="flex h-full min-h-0 min-w-0 max-h-[calc(100dvh-6rem)] flex-1 flex-col gap-4 overflow-hidden">
        ...
        >
          <ResizablePanel defaultSize={31} minSize={24}>
            <div className="flex h-full min-h-0">
              <InterviewPanelCard desktop title="Session Control Plane">
                {sessionControlPlaneContent}
              </InterviewPanelCard>
            </div>
          </ResizablePanel>
          ...
          <ResizablePanel defaultSize={36} minSize={26}>
            <div className="flex h-full min-h-0">
              <InterviewPanelCard desktop title="Transcript Monitor">
                {transcriptMonitorContent}
              </InterviewPanelCard>
            </div>
          </ResizablePanel>
          <ResizableHandle withHandle />
          <ResizablePanel defaultSize={33} minSize={24}>
            <div className="flex h-full min-h-0">
              <InterviewPanelCard desktop title="AI Answers">
                {aiAnswersContent}
              </InterviewPanelCard>
            </div>
          </ResizablePanel>
      ...
      <div className="space-y-4 xl:hidden">
        <InterviewPanelCard title="AI Answers">
          {aiAnswersContent}
        </InterviewPanelCard>
        <InterviewPanelCard title="Transcript Monitor">
          {transcriptMonitorContent}
        </InterviewPanelCard>
        <InterviewPanelCard title="Session Control Plane">
          {sessionControlPlaneContent}
        </InterviewPanelCard>
      </div>
    </section>
