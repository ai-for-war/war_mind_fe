...
type StockAgentConversationListItemProps = {
  conversation: StockAgentConversationListItem
  isActive: boolean
...
export const StockAgentConversationListItemRow = ({
  conversation,
  isActive,
  onSelect,
}: StockAgentConversationListItemProps) => (
  <button
    className={cn(
  ...
  >
    ...
    <div className="mt-2 flex items-center gap-2">
      {conversation.status === "archived" ? (
        <Badge className="h-5 px-2 text-[10px] uppercase tracking-wide" variant="outline">
          Archived
        </Badge>
      ) : null}
