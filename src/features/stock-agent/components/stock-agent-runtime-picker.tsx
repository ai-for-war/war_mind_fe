...
type StockAgentRuntimePickerProps = {
  ...
  selection: StockAgentRuntimeSelection | null
}
...
export const StockAgentRuntimePicker = ({
  catalog,
  className,
  isLoading,
  isRetrying = false,
  ...
  const selectedModel = useMemo(() => getSelectedModel(catalog, selection), [catalog, selection])
  ...
  return (
    <div className="flex min-w-0 flex-wrap items-center gap-2">
      <Button
        className={cn("h-8 max-w-full justify-between gap-2 text-xs", className)}
      ...
      {hasReasoningOptions ? (
        <Select onValueChange={onSelectReasoning} value={selection?.reasoning ?? undefined}>
          <SelectTrigger aria-label="Reasoning" className="h-8 min-w-[8.5rem] text-xs" size="sm">
        ...
        <Sheet onOpenChange={setIsModelPickerOpen} open={isModelPickerOpen}>
          <SheetContent className="p-0" side="bottom">
            <SheetHeader className="border-b pb-3">
              ...
              <SheetDescription>Pick the runtime for the next stock-agent message.</SheetDescription>
            </SheetHeader>
            {pickerContent}
          </SheetContent>
        </Sheet>
        ...
        <Dialog onOpenChange={setIsModelPickerOpen} open={isModelPickerOpen}>
          <DialogContent className="overflow-hidden p-0 sm:max-w-xl">
            <DialogHeader className="border-b px-6 pt-6 pb-3">
              ...
              <DialogDescription>
                Pick the runtime for the next stock-agent message.
              </DialogDescription>
            </DialogHeader>
            {pickerContent}
          </DialogContent>
